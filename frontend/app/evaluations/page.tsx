...
import { fetchTests, fetchEvaluations, deleteEvaluation } from "@/lib/api";
...
export default function EvaluationsPage() {
  ...
  async function handleDelete(id: number) {
    ...
    try {
      ...
      setAllEvals(prev => prev.filter(e => e.id !== id));
    } catch (e: any) {
    ...
    <div className="p-8">
      <div className="mb-8">
        ...
        <p className="text-default-500 text-sm mt-1">
          {allEvals.length} evaluation{allEvals.length !== 1 ? "s" : ""} across {tests.length} test{tests.length !== 1 ? "s" : ""}
        </p>
      </div>
        ...
        <div className="space-y-3">
          {[1, 2, 3, 4].map(i => (
            <Card key={i} className="border border-default-200">
              <Card.Content className="h-16 animate-pulse bg-default-50" />
            </Card>
          ))}
        </div>
      ) : allEvals.length === 0 ? (
                ...
                <tbody>
                  {allEvals.map(ev => (
                    <tr
                      ...
                      onMouseLeave={() => { if (confirmId === ev.id) setConfirmId(null); }}
                    >
                      <td className="py-3 px-4 font-medium text-foreground">{ev.testName}</td>
                      <td className="py-3 px-4 text-default-500 text-xs">
                          ...
                          <button
                            ...
                            className={`flex items-center gap-1 text-xs font-medium transition-all rounded px-1.5 py-0.5 ${
                              confirmId === ev.id
                                ...
                                : "text-default-400 hover:text-danger"
                            }`}
                            title={confirmId === ev.id ? "Click again to confirm delete" : "Delete evaluation"}
                          >
                            <Trash2 size={12} />
                            {deleting === ev.id ? "Deleting…" : confirmId === ev.id ? "Confirm?" : "Delete"}
                          </button>
                        </div>
                      </td>
                    </tr>
